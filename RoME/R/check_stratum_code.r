...
if (FALSE){
  ...
  wd <- "D:\\Documents and Settings\\Utente\\Documenti\\GitHub\\RoME\\Test Loredana"
  ResultDataTA= read.table(file="D:\\OneDrive - Coispa Tecnologia & Ricerca S.C.A.R.L\\SAMA18\\R_MEFH\\MEDITS data\\GSA18\\TA_GSA18_1994-2020.csv", sep=";", header=T)
  ...
  check_stratum_code(ResultDataTA,Strata=MEDITS::stratification_scheme,wd,suffix)
}

check_stratum_code <- function (ResultDataTA,Strata=MEDITS::stratification_scheme,wd,suffix){
  ...
  if (!(Dataset$NUMBER_OF_THE_STRATUM[i] %in%  as.character(Strat_table$STRATUM))) {
